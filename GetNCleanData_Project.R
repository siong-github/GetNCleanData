...
subject_test <- read.table("./GetNCleanData/UCI HAR Dataset/test/subject_test.txt", sep=" ",header=FALSE)
subject_train <- read.table("./GetNCleanData/UCI HAR Dataset/train/subject_train.txt", sep=" ",header=FALSE)
subject <- rbind(subject_train, subject_test)
names(subject) <- c("SubjectId")

y_test <- read.table("./GetNCleanData/UCI HAR Dataset/test/y_test.txt", sep=" ",header=FALSE)
y_train <- read.table("./GetNCleanData/UCI HAR Dataset/train/y_train.txt", sep=" ",header=FALSE)
X_train <- read.table("./GetNCleanData/UCI HAR Dataset/train/X_train.txt", header=FALSE)
X_test <- read.table("./GetNCleanData/UCI HAR Dataset/test/X_test.txt", header=FALSE)
X_data <- rbind(X_train,X_test)
y_data <- rbind(y_train,y_test)

#Read in variables
features <- read.table("./GetNCleanData/UCI HAR Dataset/features.txt", header=FALSE)
